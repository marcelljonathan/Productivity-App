...
-- 4b. Futures trades (each is a buy or sell of an instrument; open positions are the
--     net per instrument, realized P/L computed by average-cost netting in the app).
CREATE TABLE IF NOT EXISTS public.trade_futures_trades (
  id         UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id    UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  page_id    UUID NOT NULL REFERENCES public.custom_pages(id) ON DELETE CASCADE,
  account_id UUID NOT NULL REFERENCES public.trade_accounts(id) ON DELETE CASCADE,
  instrument TEXT NOT NULL,
  side       TEXT NOT NULL DEFAULT 'buy',   -- 'buy' (long) or 'sell' (short)
...
-- 5. Indexes.
CREATE INDEX IF NOT EXISTS trade_futures_trades_page_idx    ON public.trade_futures_trades(page_id);
CREATE INDEX IF NOT EXISTS trade_futures_trades_account_idx ON public.trade_futures_trades(account_id);
...
ALTER TABLE public.trade_futures_trades ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "own trade_futures_trades" ON public.trade_futures_trades;
CREATE POLICY "own trade_futures_trades" ON public.trade_futures_trades
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
